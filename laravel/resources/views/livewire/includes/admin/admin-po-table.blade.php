<div class="flex flex-col mt-6 overflow-x-auto -mb-9">
    <div class="w-full overflow-x-auto">
        <table class="min-w-full mt-6 mb-8 text-sm text-left text-black border-2 border-black dark:text-gray-400">
                ...
                @if($adminPurchaseOrder)
                    ...
                    <tr class="text-black border-b border-black">
                        <th class="px-4 py-2 border-r border-black">
                            <input type="text" value="{{ $item->item_no }}" class="w-full text-left bg-transparent border-0 focus:outline-none" readonly>
                        </th>
                    ...
                    <tr class="border-b border-black">
                        ...
                        <td class="px-4 py-2 font-bold">₱ {{ number_format($adminPurchaseOrder->prsItems->sum('amount'), 2, '.', ',') }}</td>
                    </tr>
                @elseif($purchaseOrder)
                    @foreach ($purchaseOrder->createPr->prsItems as $item)
                    <tr class="text-black border-b border-black">
                        <th class="px-4 py-2 border-r border-black">
                            <input type="text" value="{{ $item->item_no }}" class="w-full text-left bg-transparent border-0 focus:outline-none" readonly>
                        </th>
                    ...
                    <tr class="border-b border-black">
                        <td colspan="5" class="px-4 py-2 font-bold text-right border-r border-black">Total:</td>
                        <td class="px-4 py-2 font-bold">₱ {{ number_format($purchaseOrder->createPr->prsItems->sum('amount'), 2, '.', ',') }}</td>
                    </tr>
                @else
                    <tr>
                        <td colspan="6" class="px-4 py-2 text-center">No items available</td>
                    </tr>
                @endif
            </tbody>
        </table>
