            ...
            <div class="flex flex-col ml-9 w-[84%] max-md:ml-0 max-md:w-full">
                <div class="flex flex-row items-center justify-between"> 
                    ...
                    <div class="flex gap-2 mt-10 mr-5"> 
                        <button class="min-w-[150px] h-9 justify-center font-medium text-white rounded-md shadow bg-black flex items-center">
                            <a href="{{ route('print-request', ['id' => $createPr->purchaseRequest->id]) }}" class="flex items-center justify-center w-full h-full px-2 text-sm sm:px-4 sm:text-base">
                                CREATE PURCHASE ORDER
                            </a>
                        </button>
                        <button class="w-[74px] h-9 justify-center font-medium text-white rounded-md shadow bg-indigo-800 flex items-center">
                            <a href="{{ route('print-request', ['id' => $createPr->purchaseRequest->id]) }}" class="flex items-center justify-center w-full h-full px-4">
                                PRINT
                            </a>
                        </button>
                    </div>
